	...
	jQuery.fn.extend({
		...
		revolution: function(options) {
				...
				var defaults = {
					...
					navigationType:"bullet",				// bullet, thumb,
															// none
					navigationArrows:"solo",				// nextto, solo,
															// none
					...
					touchenabled:"on",						// Enable Swipe
															// Function : on/off
					onHoverStop:"on",						// Stop Banner Timet
															// at Hover on Slide
															// on/off


					stopAtSlide:-1,							// Stop Timer if
															// Slide "x" has
															// been Reached. If
															// stopAfterLoops
															// set to 0, then it
															...
															// has no sinn in
															// this case.
					stopAfterLoops:-1,						// Stop Timer if All
															// slides has been
															// played "x" times.
															// IT will stop at
															// THe slide which
					...
					hideCaptionAtLimit:0,					// It Defines if a
															// caption should be
															// shown under a
															// Screen Resolution
															// ( Basod on The
															// Width of Browser)
					hideAllCaptionAtLimit:0,				// Hide all The
															// Captions if Width
															// of Browser is
															// less then this
															// value
					hideSliderAtLimit:0,					// Hide the whole
															// slider, and stop
															// also functions if
															// Width of Browser
															// is less than this
					...
					shadow:0,								// 0 = no Shadow,
															...
															// Fullwidth Version
															// !)
					fullWidth:"off",						// Turns On or Off
															// the Fullwidth
															// Image Centering
															// in FullWidth
															// Modus
					fullScreen:"off",
					minFullScreenHeight:0,					// The Minimum
															// FullScreen Height
					fullScreenOffsetContainer:"",			// Size for
															// FullScreen Slider
															// minimising
															// Calculated on the
															// Container sizes
					fullScreenOffset:"0",					// Size for
															// FullScreen Slider
															// minimising
					...
					forceFullWidth:"off",						// Force The
																// FullWidth

					spinner:"spinner0",

					swipe_treshold : 75,					// The number of
															// pixels that the
															// user must move
															// their finger by
															// before it is
															// considered a
															// swipe.
					swipe_min_touches : 1,					// Min Finger
															// (touch) used for
															// swipe
					drag_block_vertical:false,				// Prevent Vertical
															// Scroll during
															// Swipe
					isJoomla:false,
					parallax:"off",
					...
					startDelay:0		// Delay before the first Animation
										// starts.
		...
		function initSlider(container,opt) {
						...
						if (!container.hasClass("revslider-initialised")) {
									...
									// CHECK IF FIREFOX 13 IS ON WAY.. IT HAS A
									// STRANGE BUG, CSS ANIMATE SHOULD NOT BE
									// USED
									...
									container.find('.tp-caption').each(function(i) {
										// IF SRC EXIST, RESET SRC'S since WE
										// DONT NEED THEM !!
									...
									// REMOVE ANY VIDEO JS SETTINGS OF THE VIDEO
									// IF NEEDED
									...
									// WE NEED TO ADD A BASIC CLASS FOR
									// SETTINGS.CSS
									...
									// A BASIC GRID MUST BE DEFINED. IF NO
									// DEFAULT GRID EXIST THAN WE NEED A DEFAULT
									// VALUE, ACTUAL SIZE OF CONAINER
									...
									// IF RESIZED, NEED TO STOP ACTUAL
									// TRANSITION AND RESIZE ACTUAL IMAGES
									...
									// CHECK IF THE CAPTION IS A "SCROLL ME TO
									// POSITION" CAPTION IS
 ...
 ******************************************************************************/
			...
			if (notIE && !isChromium) {
			    ...
			    if (window.addEventListener) {
			        ...
			        window.addEventListener("focus", function (event) {
						if (container==undefined) return false;
			            setTimeout(function(){
			                 // TAB IS ACTIVE, WE CAN START ANY PART OF THE
								// SLIDER
				            if (opt.nextSlideOnWindowFocus=="on") container.revnext();
							container.revredraw();
			            },300);
		...
		var prepareSlides = function(container,opt) {
			...
			// PREPARE THE UL CONTAINER TO HAVEING MAX HEIGHT AND HEIGHT FOR ANY
			// SITUATION
			...
			container.find('>ul:first >li').each(function(j) {
				...
				// IF LINK ON SLIDE EXISTS, NEED TO CREATE A PROPER LAYER FOR
				// IT.
		...
		/*
		 * if (opt.ffnn == undefined) opt.ffnn=0; comingtransition=opt.ffnn;
		 * 
		 * if ( direction==1) opt.ffnn=opt.ffnn-1; else opt.ffnn=opt.ffnn+1;
		 * 
		 * if (opt.ffnn>46) opt.ffnn=0; if (opt.ffnn<0) opt.ffnn = 46;
		 * 
		 * jQuery('.logo').html('Next Anim:'+comingtransition);
		 * 
		 * 
		 * if (comingtransition=="boxslide" || comingtransition == "boxfade" ||
		 * comingtransition == "papercut" || comingtransition==0 ||
		 * comingtransition == 1 || comingtransition == 16) comingtransition =
		 * 9;
		 */


			/*
			 * Transition Name , Transition Code, Transition Sub Code, Max
			 * Slots, MasterSpeed Delays, Preparing Slots (box,slideh, slidev),
			 * Call on nextsh (null = no, true/false for visibility first
			 * preparing), Call on actsh (null = no, true/false for visibility
			 * first preparing),
			 */
			...
			// joomla only change: avoid problematic transitions that don't
			// compatible with mootools
			...
			if (nexttrans==0) {								// BOXSLIDE
						...
						// ALL NEW SLOTS SHOULD BE SLIDED FROM THE LEFT TO THE
						// RIGHT
			...
			if (nexttrans==12 || nexttrans==13 || nexttrans==14 || nexttrans==15) {
						...
						// DEPENDING ON EXTENDED SPECIALS, DIFFERENT SCALE AND
						// OPACITY FUNCTIONS NEED TO BE ADDED
						if (specials == 1) op = 0;
						if (specials == 2) op = 0;
						if (specials == 3) {
								easeitout = punchgs.Power2.easeInOut;
			...
			// /////////////////////////////////////
			if (nexttrans==19 || nexttrans==22) {								// IN
																				// CUBE
			...
			// THE SLOTSLIDE - TRANSITION XX. //
			// /////////////////////////////////////
			if (nexttrans==23 || nexttrans == 24) {								// cube-horizontal
																				// -
																				// inboxhorizontal
		...
		var vimeoready_auto = function(player_id,autoplay) {
			...
			var updateHTML5Size = function(pc,container) {
				...
				var animateTheCaptions = function(nextli, opt,recalled,mtl) {
						...
						// MAKE SURE THE ANIMATION ENDS WITH A CLEANING ON MOZ
						// TRANSFORMS
						...
						allcaptions.each(function(i) {
								...
								if (nextcaption.hasClass("tp-static-layer")) {
									...
									// if (staticdirection==2) staticdirection =
									// 0;
								...
								if (handlecaption==0) {
									...
									if (nextcaption.hasClass("tp-videolayer") || nextcaption.find('iframe').length>0 || nextcaption.find('video').length>0 ) {
										...
										if ((videomp!=undefined || videowebm!=undefined) && nextcaption.find('video').length==0) {
											...
											/*
											 * if
											 * (nextcaption.data('videoposter')!=undefined)
											 * apptxt = apptxt +
											 * 'poster="'+nextcaption.data('videoposter')+'">';
											 * apptxt = apptxt + '<source
											 * src="'+videomp+'"
											 * type="video/mp4" />'; apptxt =
											 * apptxt + '<source
											 * src="'+videowebm+'"
											 * type="video/webm" />'; apptxt =
											 * apptxt + '<source
											 * src="'+videoogv+'"
											 * type="video/ogg" />'; apptxt =
											 * apptxt + '</video>';
										...
										// IF AUTOPLAY IS ON, WE NEED SOME STOP
										// FUNCTION ON
										if (nextcaption.data('autoplay')==true) {
											setTimeout(function() {
									...
									if (nextcaption.data('x')=="center" || nextcaption.data('xcenter')=='center') {
										...
										// nextcaption.data('x',(crw/2 -
										// nextcaption.outerWidth(true)/2)/xbw+
										// hofs);
									...
									if (nextcaption.data('x')=="right" || nextcaption.data('xright')=='right') {
										...
										// konsole.log("crw:"+crw+"
										// width:"+nextcaption.outerWidth(true)+"
										// xbw:"+xbw);
							...
							if (!internrecalled) {
									...
									// CLEAR THE TIMELINE, SINCE IT CAN BE
									// DAMAGED, OR PAUSED AT A FEW PART
									...
									if (staticdirection != 2) {
										...
										// CHANGE to punchgs.TweenLite. if Yoyo
										// and Repeat is used. Dont forget to
										// laod the Right Tools for it !!
									...
									// SAVE IT TO NCAPTION BEFORE NEW STEPS WILL
									// BE ADDED
									nextcaption.data('timeline',tl);

									// FURTHER ANIMATIONS IN CASE THERE ARE MORE
									// THAN ONE STEP IN THE ANIMATION CHAIN
									...
									// For Static Layers -> 1 -> In, 2-> Out 0->
									// Ignore -1-> Not Static
				...
				var callCaptionLoops = function(nextcaption,factor) {
									...
									// SOME LOOPING ANIMATION ON INTERNAL
									// ELEMENTS
				...
				var removeTheCaptions = function(actli,opt) {
						...
						allcaptions.each(function(i) {
							    ...
							    var staticdirection = -1;	// 1 -> In, 2-> Out
															// 0-> Ignore -1->
															// Not Static
							...
							if (staticdirection != 0 ) {
									...
									if (nextcaption.find('iframe').length>0) {
																	...
																	// PAUSE
																	// if
																	// (nextcaption.data('vimeoid')!=undefined
																	// &&
																	// String(nextcaption.data('vimeoid')).length>0)
									...
									try {
											...
											// var tl =
											// punchgs.TimelineLite.exportRoot();
											var tl = nextcaption.data('timeline');
											var endstarts = tl.getLabelTime("frame99");
											var curtime = tl.time();
											if (endstarts>curtime) {

												// WE NEED TO STOP ALL OTHER
												// NIMATIONS
			...
			nextsh.find('.defaultimg').each(function() {
				...
				if (prepareonly) {
					...
					defimg.data('kenburn',punchgs.TweenLite.fromTo(kbimg,dur,{autoAlpha:1, force3D:punchgs.force3d, transformOrigin:"0% 0%", top:0,left:0, scale:imgs.w, x:imgs.x, y:imgs.y},{autoAlpha:1,rotationZ:roe,ease:easeme, x:imge.x, y:imge.y,scale:imge.w,onUpdate:function() {
							...
							if (!isIE(8)) defimg.data('currotate',getRotationDegrees(kbimg));
							if (!isIE(8)) defimg.data('curscale',(imgobj.w*sx)+"%  "+(imgobj.h*sx+"%"));
							// punchgs.TweenLite.set(defimg,{rotation:defimg.data('currotate'),
			...
			if (opt.parallax=="mouse" || opt.parallax=="scroll+mouse" || opt.parallax=="mouse+scroll") {
						...
						if (is_mobile())
							window.ondeviceorientation = function(event) {
							  ...
							  // jQuery('.logo-container').html("h:"+curh+"
								// v:"+curv);
							  }
			}
			if (opt.parallax=="scroll" || opt.parallax=="scroll+mouse" || opt.parallax=="mouse+scroll") {
